...
def is_finger_close_to_box(finger_position, box_coords):
    ...
    return (x_min - CLOSE_DISTANCE_THRESHOLD <= finger_x <= x_max + CLOSE_DISTANCE_THRESHOLD and
            y_min - CLOSE_DISTANCE_THRESHOLD <= finger_y <= y_max + CLOSE_DISTANCE_THRESHOLD)

def process_hand_gestures(frame):
    """Process the frame for hand gestures and return the modified frame."""
    rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results = hands.process(rgb_frame)

    if results.multi_hand_landmarks:
        hand_states = []  # To store finger states for each hand
